...
double cte_image_neo::get_difftime( struct timeval start, struct timeval end )
{
  double seconds;
  double microseconds;
...
long cte_image_neo::get_sparse_pixels( std::valarray<double> & v, double limit )
{
  long nr = 0;
...
void cte_image_neo::limit_to_max( std::valarray<double> & v, double limit )
{
  for (unsigned long i=0;i<v.size();i++)
    if ( v[i] > limit )
      v[i] = limit;
...
double cte_image_neo::get_sum_double_array( double *array, int w, int h )
{
  double sum = 0.0;
...
void cte_image_neo::print_traps( std::valarray<double> & t, int n_species, int trap_levels )
{
  int i, j;
...
void cte_image_neo::print_wml( std::valarray<std::valarray<double>> & wml,
...
bool cte_image_neo::val_array_smaller( std::valarray<double> & v1,
...
void cte_image_neo::create_express_multiplier( std::valarray<int> & express_multiplier,
                                           int express,
                                           int h,
                                           int readout_offset )
{
...
void cte_image_neo::clock_charge_image_classic( std::valarray<double> & image,
...
void cte_image_neo::clock_charge_image_neo( std::valarray<double> & image,
				    std::valarray<long> & xrange,
				    std::valarray<long> & yrange )
{
  // local variables copied from parameter set
...
void cte_image_neo::clock_charge_image_neo2( std::valarray<double> & image,
				    std::valarray<long> & xrange,
				    std::valarray<long> & yrange )
{
  // local variables copied from parameter set
...
void cte_image_neo::clock_charge( std::shared_ptr<std::valarray<double>> im,
			      long w, long h,
			      std::valarray<long> & xrange,
			      std::valarray<long> & yrange )
{
  output( 11, "cte_image_neo::clock_charge\n" );
